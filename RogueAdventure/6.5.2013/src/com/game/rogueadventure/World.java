...
public class World
{
	...
	private void buildWorld()
	{
		...
		for(int x = 0 ; x < width ; x++)
		{
			...
			{
				blocks[x][y] = new Block(x,y,Tile_Type.WALL);
			}
		}
		for(int x = 0 ; x < width ; x++ ) //creates walls around room
		{
			for(int y = 0 ; y < height ; y++)
			{
				...
				{
					blocks[x][y].type = Tile_Type.WALL;
				}
				else
				{
					if( x == 0 || x == width-1 )
   ...
   {
       blocks[rooms[getRoomCount()-1].centerX][rooms[getRoomCount()-1].centerY].setType(Tile_Type.DOWNSTAIRS);
   }
   
   private void placeUpstairCase()
   {
       blocks[rooms[getRoomCount()-1].centerX][rooms[0].centerY].setType(Tile_Type.UPSTAIRS);
   }
