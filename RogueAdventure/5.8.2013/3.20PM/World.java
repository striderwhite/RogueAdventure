...
public class World implements Parcelable
{
	...
	//this reconstructs the world (parcel)
	World(Parcel p)
	{
        tiles = (Tile[][]) p.readSerializable();
        rooms = (Room[]) p.readSerializable();
        roomsToMake = p.readInt();
        height = p.readInt();
        width = p.readInt();
        tileSize = p.readInt();
	}
	...
	private void buildWorld()
	{
		...
		for(int x = 0 ; x < width ; x++)
		{
			...
			{
				tiles[x][y] = new Tile();
				tiles[x][y].invisible = false;
                tiles[x][y].wall = true;
			}
		}
		
		//creates a boundry of walls
		for(int x = 0 ; x < width ; x++ )
		{
			for(int y = 0 ; y < height ; y++)
			{
	...
	private void createRooms()
	{
	    ...
	    roomsToMake = gen.nextInt(7) + 4; //make between 4 and 7 rooms
		...
		for(int currentRoom = 0 ; currentRoom < getRoomCount() ; currentRoom++)
		{
			...
			while(roomXIndex < roomMaxX)
			{
			    ...
			    while(roomYIndex < roomMaxY)
			    {
			        ...
			        if(worldX > 29){ worldX = 29; }
	                if(worldY > 29){ worldY = 29; }
	...
	private void connectRooms()
	{
		for(int room = 0 ; room < getRoomCount() - 1 ; room++) //-1 because i look ahead one cell in the array when comparing
		{
			...
			for(int x = 0 ; x < Math.abs(diffx) ; x++)
			{
				...
				tiles[xTile][yTile].invisible = false;
				tiles[xTile][yTile].wall = false;
			 }
	...
	protected void add(Tile tile, int x, int y)
	{
	    tiles[x][y] = tile;
	}
    ...
    {
        p.writeSerializable(tiles);
        p.writeSerializable(rooms);
        p.writeInt(roomsToMake);
        p.writeInt(height);
        p.writeInt(width);
        p.writeInt(tileSize);
    }
