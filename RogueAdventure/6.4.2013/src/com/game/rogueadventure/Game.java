...
public class Game extends Activity 
{
	...
	Button zoomOut, zoomIn, up, down, left, right, upLeft, downLeft, upRight, downRight, attack, load, newGame, slot1, slot2, slot3, inventory, magic;
	...
	private void init()
	{
        ...
        manaView.setText("Mana " + player.mana + "/" + player.maxMana);
        expView.setText("Exp 0/" + player.expBeforeNextLevel);
        
        if(player.typeBeforeEntityMoved == Tile_Type.WATER ||  player.typeBeforeEntityMoved == Tile_Type.GRASS ||  player.typeBeforeEntityMoved == Tile_Type.ICE)
        {
        ...
        manaBar.setMax(player.maxMana);
        expBar.setMax(player.expBeforeNextLevel);
        
        healthBar.setProgress(player.maxHealth);
        manaBar.setProgress(player.maxMana);
	...
	{ 
        ...
        {
            eventLogTextView.logText("info  " , "Level up! Now level " + player.level + " now " + player.expBeforeNextLevel + " exp needed until level " + (player.level+1));
        }
        ...
        expView.setText("Exp " + player.experience + "/" + player.expBeforeNextLevel);
        healthView.setText("Health " + player.health + "/" + player.maxHealth);
        world.remove(e);
    }
    ...
    private void centerCamera()
    {
        ...
        int offsetX = 80 + world.tileSize;
        int offsetY = 10 + world.tileSize;
        
        int screenWidth = size.x;
        int screenHeight = size.y;
        int playerDistanceFromOriginX = (world.tileSize * player.x) + ( (graphics.TILE_SPACING) * player.x);
        int playerDistanceFromOriginY = (world.tileSize * player.y) + ( (graphics.TILE_SPACING) * player.y);
    ...
    public void getViewPointers()
    {
        ...
        magic = (Button) findViewById(R.id.MagicBtn);
