...
public class World
{
	...
	private void buildWorld()
	{
		...
		for(int x = 0 ; x < width ; x++)
		{
			...
			{
				tiles[x][y] = new Tile(Tile_Type.WALL);
			}
		}
	...
	private void populateWorld()
	{
	    ...
	    int enemiesToMake = gen.nextInt(5) + 2; //between 2 and 5 enemies
	    int roomToSpawnIn;
	    enemies = new ArrayList<Enemy>(enemiesToMake);
	    for(int x = 0 ; x < enemiesToMake ; x++)
	    {
	        roomToSpawnIn = gen.nextInt(getRoomCount() - 1) + 1;
	        enemies.add(new Enemy(rooms[roomToSpawnIn].centerX,rooms[roomToSpawnIn].centerY, this ,Tile_Type.ENEMY));
	    }
	}
